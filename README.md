...
the 30-question set, so every figure in that section is in-sample. A second set
exists to test those choices out of sample.

45 questions, generated by a different model from 203 sections the tuning set
never touched, with keys derived mechanically from the section each question was
written from and committed before the first run. Graded by **Opus 5, blind** —
it sees the question, the answer and the source text, never the pipeline and
never whether retrieval succeeded.
...
out of sample it climbs from 26/45 to 36/45 at k=20 with no elbow. And the blind
grader counted 11 refusals where the code counted 8 — three answers declined
without the exact phrase the refusal check matches on, a detection gap now
fixed. Both are in [FINDINGS.md](FINDINGS.md).

### The tuning set: 30 questions with keys written from the sources

Committed before the first run, and read by hand against the sources. The same
answers were then re-graded blind by a separate model. **Both grades are
reported side by side** — the blind grader was stricter on five.
...
The gap is a rubric difference rather than an error. An answer that flags its
own incompleteness reads as correct under one rubric and partial under another;
both are defensible. Reporting both takes the choice out of the author's hands,
which is the point of grading it twice.
